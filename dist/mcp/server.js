...
var import_zod, import_promises, import_fs, import_path, CustomCheckSchema, CustomPrincipleSchema, DEFAULT_CALM_PRECISION_CONFIG, CalmPrecisionConfigSchema, TypographyTokensSchema, DesignSystemConfigSchema;
var init_config = __esm({
  "src/design-system/config.ts"() {
    ...
    CustomCheckSchema = import_zod.z.object({
    ...
    CustomPrincipleSchema = import_zod.z.object({
    ...
    CalmPrecisionConfigSchema = import_zod.z.object({
      core: import_zod.z.array(import_zod.z.string()).default(DEFAULT_CALM_PRECISION_CONFIG.core),
      stylistic: import_zod.z.array(import_zod.z.string()).default(DEFAULT_CALM_PRECISION_CONFIG.stylistic),
      severity: import_zod.z.record(import_zod.z.string(), import_zod.z.enum(["error", "warn", "off"])).default({})
    });
    TypographyTokensSchema = import_zod.z.object({
    ...
    DesignSystemConfigSchema = import_zod.z.object({
...
var import_zod2, ExtendedTokenSpecSchema;
var init_schema = __esm({
  "src/design-system/tokens/schema.ts"() {
    "use strict";
    import_zod2 = require("zod");
    ExtendedTokenSpecSchema = import_zod2.z.object({
...
var import_fs2, touchTargetValidator, fontSizeValidator, colorValidator, cornerRadiusValidator, spacingValidator, tokenValidators;
...
var import_promises3, import_path2, SELECTOR_ACTIONABILITY_PROBE_FN, CompatElementHandle, CompatLocator, ESBUILD_NAME_HELPER, CompatPage;
...
var init_schemas = __esm({
  "src/schemas.ts"() {
    ...
    ThresholdBasisSchema = import_zod3.z.enum(["research", "internal-testing", "hypothesis"]);
    ProvenancedThresholdSchema = import_zod3.z.object({
      value: import_zod3.z.number(),
      basis: ThresholdBasisSchema,
      rationale: import_zod3.z.string().min(1),
      source: import_zod3.z.string().optional(),
      reviewedAt: import_zod3.z.string()
    });
    VerdictPolicySchema = import_zod3.z.object({
    ...
    ThresholdOverrideSchema = import_zod3.z.union([
      import_zod3.z.number(),
      ProvenancedThresholdSchema.partial()
    ]);
    VerdictPolicyOverrideSchema = import_zod3.z.object({
    ...
    ChangedRegionSchema = import_zod3.z.object({
       ...
       */
      diffPercent: import_zod3.z.number().optional()
    });
    ...
    InteractiveStateSchema = import_zod3.z.object({
      ...
      isContentEditable: import_zod3.z.boolean().optional()
    });
    A11yAttributesSchema = import_zod3.z.object({
      ...
      ariaCurrent: import_zod3.z.string().nullable().optional(),
    ...
    BreadcrumbContextSchema = import_zod3.z.object({
      rootSelector: import_zod3.z.string(),
      rootTag: import_zod3.z.string(),
      rootRole: import_zod3.z.string().nullable(),
      accessibleName: import_zod3.z.string().nullable(),
      listTag: import_zod3.z.string().nullable(),
      itemCount: import_zod3.z.number(),
      linkCount: import_zod3.z.number(),
      currentValues: import_zod3.z.array(import_zod3.z.string()),
      currentPageCount: import_zod3.z.number(),
      currentPageIsLast: import_zod3.z.boolean(),
      lastItemIsLink: import_zod3.z.boolean(),
      representative: import_zod3.z.boolean()
    });
    ...
    EnhancedElementSchema = import_zod3.z.object({
      ...
      inForm: import_zod3.z.boolean().optional(),
...
var import_child_process3, import_util2, import_promises11, import_path11, execFileAsync2;
...
function resolvePluginPaths(pluginPath) {
  ...
  return { bundlePath, stylesPath: (0, import_node_fs2.existsSync)(stylesPath) ? stylesPath : void 0 };
}
...
init_extract3();
var execFileAsync8 = (0, import_util8.promisify)(import_child_process10.execFile);
var SETTLE_MS = 220;
...
init_macos();
var execFileAsync9 = (0, import_util9.promisify)(import_child_process11.execFile);
...
init_extract3();
var execFileAsync10 = (0, import_util10.promisify)(import_child_process12.execFile);
var SETTLE_MS2 = 250;
...
var ResolvedPathCache = class _ResolvedPathCache {
  ...
  set(sessionId, target, path, signature) {
...
async function readScrollPosition(driver2) {
  if (typeof driver2.evaluate !== "function") return null;
  try {
    const raw = await driver2.evaluate("({ x: window.scrollX, y: window.scrollY })");
    ...
    switch (name) {
      ...
      case "interact": {
        ...
        try {
          ...
          const beforeUrl = driver2.url;
          const captured = await driver2.actAndCapture(async () => {
            switch (action) {
              case "click":
      ...
      case "session_action": {
        ...
        if (action === "scroll" && isPageScrollTarget(target)) {
          try {
            ...
            const before = await readScrollPosition(driver2);
            await driver2.scroll(deltaY);
            await new Promise((r) => setTimeout(r, 300));
            const after = await readScrollPosition(driver2);
            const afterEls = await driver2.getSnapshot();
...
${meta.links.slice(0, 20).map((l) => `  \u2022 ${l.label}`).join("\n")}${meta.links.length > 20 ? `
  ... and ${meta.links.length - 20} more` : ""}`);
            ...
            case "screenshot": {
              ...
              return imageResponse2(base64, `Screenshot of ${driver2.url}`);
            }
            case "state": {
              const elements = await driver2.getSnapshot();
              const elementCount = elements.filter((e) => e.actions.length > 0).length;
...
async function handleNativeCompare(args) {
  ...
  if (sessionId) {
    ...
    session = await getSession2(DEFAULT_OUTPUT_DIR2, sessionId);
    if (!session) {
      return errorResponse2(`Session "${sessionId}" not found.`);
    }
  } else {
  ...
  switch (action) {
    case "tap": {
      ...
      try {
        ...
        const { extractNativeElements: extractNativeElements2, isExtractorAvailable: isExtractorAvailable2 } = await Promise.resolve().then(() => (init_extract3(), extract_exports));

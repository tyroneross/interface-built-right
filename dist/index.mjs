...
var init_schemas = __esm({
  "src/schemas.ts"() {
    ...
    ThresholdBasisSchema = z.enum(["research", "internal-testing", "hypothesis"]);
    ProvenancedThresholdSchema = z.object({
      value: z.number(),
    ...
    ThresholdOverrideSchema = z.union([
      z.number(),
      ProvenancedThresholdSchema.partial()
    ]);
    VerdictPolicyOverrideSchema = z.object({
    ...
    ComparisonResultSchema = z.object({
      match: z.boolean(),
    ...
    SessionStatusSchema = z.enum(["baseline", "compared", "pending"]);
    ...
    LandmarkElementSchema = z.object({
      name: z.string(),
      // e.g., 'logo', 'header', 'nav'
      selector: z.string(),
    ...
    ComparisonReportSchema = z.object({
      sessionId: z.string(),
    ...
    InteractiveStateSchema = z.object({
    ...
    EnhancedElementSchema = z.object({
      // Identity
      selector: z.string(),
      tagName: z.string(),
    ...
    RuleSeveritySchema = z.enum(["off", "warn", "error"]);
    RuleSettingSchema = z.union([
      RuleSeveritySchema,
      z.tuple([RuleSeveritySchema, z.record(z.string(), z.unknown())])
    ]);
    RulesConfigSchema = z.object({
      extends: z.array(z.string()).optional(),
      rules: z.record(z.string(), RuleSettingSchema).optional()
    });
    ViolationSchema = z.object({
      ruleId: z.string(),
      ruleName: z.string(),
    ...
    RuleAuditResultSchema = z.object({
      url: z.string(),
      timestamp: z.string(),
    ...
    MemorySourceSchema = z.enum(["user", "learned", "framework"]);
    PreferenceCategorySchema = z.enum([
      "color",
      "layout",
      "typography",
      "navigation",
    ...
    ExpectationOperatorSchema = z.enum(["equals", "contains", "matches", "gte", "lte"]);
    ExpectationSchema = z.object({
      property: z.string(),
    ...
    LearnedExpectationSchema = z.object({
      id: z.string(),
      sessionId: z.string(),
      route: z.string(),
    ...
    ActivePreferenceSchema = z.object({
      id: z.string(),
      description: z.string(),
      category: PreferenceCategorySchema,
      route: z.string().optional(),
    ...
    DesignSystemViolationSchema = z.object({
    ...
    DesignSystemResultSchema = z.object({
      configName: z.string(),
...
var init_config = __esm({
  "src/design-system/config.ts"() {
    ...
    CalmPrecisionConfigSchema = z.object({
    ...
    TypographyTokensSchema = z.object({
...
var init_runtime_path = __esm({
  "src/native/runtime-path.mts"() {
    moduleDir = typeof __dirname === "string" ? __dirname : dirname(fileURLToPath(import.meta.url));
  }
});
...
async function scanNative(options = {}) {
  ...
  if (screenshot) {
    ...
    const ssPath = join(outputDir, "native", `${device.udid.slice(0, 8)}-${timestamp2}.png`);
...
var ExternalActionEvidenceInputSchema = z.object({
...
var surfaceReceiptSchema = z.object({
  kind: z.enum(["native", "web"]),
  pid: z.number().int().positive().optional(),
  bundleId: z.string().regex(SAFE_TOKEN).optional(),
...
var ExternalActionReceiptSchema = z.object({
    ...
    while (queue.length > 0 && discovered.size < maxPages) {
      ...
      try {
        ...
        for (const link2 of links) {
          if (discovered.size >= maxPages) break;
          try {
            const absoluteUrl = new URL$1(link2.href, current.url);
...
async function getNavigationLinks(url) {
  ...
  try {
    ...
    for (const link2 of navLinks) {
      try {
        const absoluteUrl = new URL$1(link2.href, url);
...
var NumberRuleSchema = z.discriminatedUnion("mode", [
...
var DesignElementSchema = z.object({
  id: z.string().min(1),
  sourceNode: z.object({
    id: z.string(),
    name: z.string(),
...
init_extract3();
init_role_map();
var execFileAsync7 = promisify(execFile);
...
// src/native/keyboard.ts
var execFileAsync8 = promisify(execFile);
var SETTLE_MS = 220;
...
init_macos();
var execFileAsync9 = promisify(execFile);
...
// src/native/menu.ts
init_extract3();
var execFileAsync10 = promisify(execFile);
var SETTLE_MS2 = 250;
...
var execFileAsync11 = promisify(execFile);
...
async function macOSNativePreflight(options) {
  ...
  const extractorPath = join(process.cwd(), ".ibr", "bin", "ibr-ax-extract");
  const swiftSourceDir = join(process.cwd(), "src", "native", "swift", "ibr-ax-extract");
...
async function compare(options) {
  ...
  await mkdir(outputDir, { recursive: true });
  const timestamp2 = Date.now();
  const actualBaselinePath = baselinePath || join(outputDir, `baseline-${timestamp2}.png`);
  const actualCurrentPath = currentPath || join(outputDir, `current-${timestamp2}.png`);
  const diffPath = join(outputDir, `diff-${timestamp2}.png`);
  if (url && !baselinePath) {

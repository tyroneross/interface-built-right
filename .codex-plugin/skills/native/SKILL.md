...
6. `native_session_action` also accepts `keystroke` (chord to the focused element, e.g. `chord: "Meta+n"`, `target` optional) — this is live. `app` (lifecycle: `op: "launch"|"switch"|"quit"`) and `menuPath` (AXMenu traversal) are on the schema but return a structured `not-implemented` outcome until their backend capability lands; do not report them as working. Check `validator.passed` on the response, not just that the call returned — `success` is only `true` when the AX-state validator passed.

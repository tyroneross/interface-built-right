...
**Layout overflow (web scan):** plain `ibr scan` (not just `scan_obsidian`) runs the same detector rooted at `body`. `result.layoutOverflow` returns `LayoutOverflowFinding[]` with kinds `self-overflow`, `container-escape`, `sibling-overlap` (all three as above) plus `clip` — text whose measured `Range` rect spills past the nearest ancestor that actually hides it (`overflow: hidden`/`clip`, stopping the walk at `scroll`/`auto`), skipped when `text-overflow: ellipsis` or a line-clamp is declared, or the clip box is sr-only-sized (≤2px). Every finding is also pushed into `issues[]` (category `structure`, severity from the finding). On by default; disable per-call with CLI `--no-layout-overflow` or MCP `scan` tool input `layout_overflow: false`. `layoutCollisions` (above) still covers INTERACTIVE elements only — `layoutOverflow` covers the whole page, including plain text.

...
import {
  ...
  runMacOSSessionAction,
  runSimulatorSessionAction,
  readMacOSSession,
  readSimulatorSession,
...
// `sessions` map and `__test_setSession` now live in ./sessions.js (imported
// and re-exported above). Native session handlers live in ./native-tools.js.
...
async function handleNativeCompare(
  ...
  // top/middle/bottom region semantics (Defect 3) plus the native verdict-policy
  // preset so the numeric boundaries and their echoed provenance are app-type
  // appropriate for iPhone/watch/mac screenshots.
